...
def update_seasonal():
    ...
    with open('./seasonal.txt', 'r', encoding='utf-8') as fp:
        ...
        while line:
            ...
            if '档名' in _line:
                ...
                if s_id not in voice_data:
                    voice_data[s_id] = {}
                if s_id not in voice_data:
                    voice_data[s_id] = {}
                if v_id not in voice_data[s_id]:
                    voice_data[s_id][v_id] = {}
                cur_suff = suff
            elif '中文译文' in _line:
                if not cur_shipid:
                    line = fp.readline()
                ...
                s_id = str(cur_shipid)
                v_id = str(cur_voiceid)
                voice_data[s_id][v_id][cur_suff] = translation
                cur_shipid = None
                cur_voiceid = None
            line = fp.readline()
    ...
    for api_id, voices in voice_data.items():
        if api_id not in quotes_size_json:
            continue
        qv_keys = quotes_size_json[api_id].keys()
        for v_id, seasonals in voices.items():
            if v_id in qv_keys:
                seasonal_keys = list(quotes_size_json[api_id][v_id].values())
                desc = id2Desc[int(v_id)]
                for seasonal_key in seasonals:
                    if seasonal_key in seasonal_keys:
                        content = voices[v_id][seasonal_key].strip()
                        if desc in quotes[api_id] and quotes[api_id][desc] == content:
                            continue
                        vid_seasonal = '{}@{}'.format(v_id, seasonal_key)
                        quotes[api_id][vid_seasonal] = content
...
def repair_subtitles():
    ...
    suffix_table = {}
    if os.path.isfile('./repair.json'):
        with open('./repair.json', 'r', encoding='utf-8') as fp:
            suffix_table = json.load(fp)
    else:
        with open('./repair.json', 'w', encoding='utf-8') as fp:
            json.dump(suffix_table, fp, ensure_ascii=False, indent=2)
    ...
    with open('./seasonal.txt', 'r', encoding='utf-8') as fpr:
        ...
        for subtitle in all_subtitles:
            subtitle_idx += 1
            idx = str(subtitle_idx)
            if idx in suffix_table:
                if suffix_table[idx]['change']:
                    res = 'No.{} "{}" ~> "{}"\n'.format(
                        str(subtitle_idx).zfill(3), suffix_table[idx]['from'], suffix_table[idx]['to'])
                    log.write(res)
                    print(res)
                    log.flush()
                continue
